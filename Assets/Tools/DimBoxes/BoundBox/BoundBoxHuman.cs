...
{
    [ExecuteInEditMode]
    public class BoundBoxHuman : MonoBehaviour
    {
        ...
        void Reset()
        {
            ...
            calculateBoundsHuman(meshRenderers);    
        ...
        void calculateBoundsHuman(SkinnedMeshRenderer[] meshRenderers)
        {
            ...
            /*isStatic = new meshes[0].GetComponent<Renderer>().isPartOfStaticBatch;
            ...
            {
                Mesh ms = meshRenderers[i].sharedMesh;
                ...
                for (int j = 0; j < vc; j++)
                {
                    if (i == 0 && j == 0)
                    {
                        meshBound = new Bounds(meshRenderers[i].transform.TransformPoint(ms.vertices[j]), Vector3.zero);
                    }
                    else
                    {
                        meshBound.Encapsulate(meshRenderers[i].transform.TransformPoint(ms.vertices[j]));
                    }
                    if (maxX < ms.vertices[j][0]) { maxX = ms.vertices[j][0]; };
                    if (maxY < ms.vertices[j][1]) { maxY = ms.vertices[j][1]; };
                    if (maxZ < ms.vertices[j][2]) { maxZ = ms.vertices[j][2]; };
                    if (minX > ms.vertices[j][0]) { minX = ms.vertices[j][0]; };
                    if (minY > ms.vertices[j][1]) { minY = ms.vertices[j][1]; };
                    if (minZ > ms.vertices[j][2]) { minZ = ms.vertices[j][2]; };
                }
                // Debug.Log(maxX - minX);
                // Debug.Log(maxY - minY);
                // Debug.Log(maxZ - minZ);
        ...
        void setPoints()
        {
            ...
            float scaleX = 0.6f;
            float scaleZ = 1.3f;
            bound.size = new Vector3(bound.size.x * transform.localScale.x / startingScale.x * scaleX, bound.size.y * transform.localScale.y / startingScale.y, bound.size.z * transform.localScale.z / startingScale.z * scaleZ);
